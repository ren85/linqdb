...
{
    public class DistributedUpdate : ITestDistributed
    {
        public void Do(DistributedDb db)
        {
            ...
            var dic = new Dictionary<DistributedId, int?>();
            dic[new DistributedId(2,2)] = 8;
            dic[new DistributedId(3,3)] = 11;
            db.DistributedTable<SomeDataDistributed>().Update(f => f.PeriodId, dic);
            ...
            res = db.DistributedTable<SomeDataDistributed>()
                        ...
                        .OrderBy(f => f.PeriodId)
                        .ToList();
            if (res.Count() != 2 || res[0].PeriodId + res[1].PeriodId != 19)
            {
                throw new Exception("Assert failure");
            }
