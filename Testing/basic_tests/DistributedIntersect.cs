...
{
    public class DistributedIntersect : ITestDistributed
    {
        public void Do(DistributedDb db)
        {
            ...
            var res = db.DistributedTable<SomeDataDistributed>()
                        .Intersect(f => f.PeriodId, new HashSet<int?>() { 7, 15 })
                        .Select(f => new
                        {
                            f.PeriodId
                        });
            if (res.Count() != 2 || res[0].PeriodId + res[1].PeriodId != 22)
            {
                throw new Exception("Assert failure");
            }
            res = db.DistributedTable<SomeDataDistributed>()
                    .Intersect(f => f.PeriodId, new List<int>() { 7, 15 })
                    .Select(f => new
                    {
                        f.PeriodId
                    });
